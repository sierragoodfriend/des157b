...
(function() {
    ...
    const leaveGameBtn = document.querySelector('#leaveGame');
    ...
    const cityImgAndHomelessRate = document.querySelector(`#city`);
    ...
    function regenerateOptions() {
        switch (choiceSequence) {
            ...
            case 3:
                ...
                break;
            case 4:
            prompt.innerHTML = `DISCLAIMER: This game is designed to spark conversation and encourage thoughtful reflection about issues surrounding homelessness and housing. The topics discussed are complex and nuanced, so please keep in mind that the answers provided are not definitive.`;
            prompt.style.maxWidth = `750px`;
            prompt.style.marginLeft = `70px`;
            prompt.style.marginTop = `40px`;
            resultsBtn.classList.remove(`hidden`);
            option1.remove();
            option2.remove();
            option3.remove();

            // displays winning results when results button is clicked, changing the background, displaying new action buttons, and telling the user how they did
            resultsBtn.addEventListener(`click`, function(){
                //use this function in each option for the if/else statements to edit the background
                function updateResultsArea() {
                    cityImgAndHomelessRate.innerHTML = ``;
                ...
                if (health >= 50) {
                    updateResultsArea();
                    prompt.innerHTML = `You're choices could have been stronger... they made homeless rates go up significantly. You can always try again!`;
                } else if (health > 15 && health < 50) {
                    updateResultsArea();
                    cityImgAndHomelessRate.innerHTML = ``;
                    prompt.innerHTML = `You're choices improved homeless rates, but they could have been stronger... You can always try again!`;
                } else if (health <= 15) {
                    updateResultsArea();
                    cityImgAndHomelessRate.innerHTML = ``;
                    prompt.innerHTML = `Wonderful job! You made choices that would have a positive impact in a real city. Congratulations!`;
                }
